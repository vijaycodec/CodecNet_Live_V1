...
// PATCH 53: Import reCAPTCHA hook (CWE-306 Fix)
import { useRecaptcha } from '@/hooks/useRecaptcha'
// PATCH 60: Import input validation (CWE-20 Fix)
import { validateLoginIdentifier, validatePassword, sanitizeString } from '@/utils/inputValidation'

const BASE_URL = process.env.NEXT_PUBLIC_RBAC_BASE_IP || 'http://localhost:5000/api' // Production backend URL
import {
  ShieldCheckIcon,
  EyeIcon,
  EyeSlashIcon,
...
export default function LoginPage() {
  ...
  // PATCH 53: Initialize reCAPTCHA hook (CWE-306 Fix)
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // PATCH 53: Execute reCAPTCHA before login (CWE-306 Fix)
    if (!isRecaptchaReady) {
    ...
    try {
      ...
      if (!recaptchaToken) {
        setError(recaptchaError || 'Security verification failed. Please refresh the page.')
        setIsLoading(false)
        return
      }

      console.log('✅ [LOGIN] reCAPTCHA token obtained, proceeding with login')
      ...
      const res = await fetch(`${BASE_URL}/auth/login`, {
        ...
        body: JSON.stringify({
          ...
          password: password,
          recaptchaToken: recaptchaToken, // PATCH 53: Include reCAPTCHA token
        }),
      })
